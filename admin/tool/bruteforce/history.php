...
if ($download) {
    ...
    $exporter->add_data(['time', 'ip', 'userid', 'username', 'event', 'reason', 'duration']);
...
$PAGE->set_url(new moodle_url('/admin/tool/bruteforce/history.php'));
$PAGE->set_context($context);
$PAGE->set_title(get_string('history', 'tool_bruteforce'));
$PAGE->set_heading(get_string('history', 'tool_bruteforce'));
...
$form .= html_writer::empty_tag('input', ['type'=>'text', 'name'=>'type', 'value'=>$type, 'placeholder'=>'type']);
$form .= html_writer::empty_tag('input', ['type'=>'submit', 'value'=>get_string('search')]);
$form .= html_writer::end_tag('form');
echo $form;

$table = new html_table();
$table->head = [get_string('time'), get_string('ip', 'tool_bruteforce'), get_string('user'), 'Event', 'Reason', 'Duration'];
foreach ($records as $r) {
    $userdisp = $r->username;
    if ($r->userid) {
...
if ($table->data) {
    ...
    $downloadurl = new moodle_url('/admin/tool/bruteforce/history.php', ['download'=>1] + $params);
